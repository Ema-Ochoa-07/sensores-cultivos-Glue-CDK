...
public class EtlCultivoSensoresStack extends Stack {
    ...
    public EtlCultivoSensoresStack(final Construct scope, final String id, final StackProps props) {
        ...
        Function riegoCultivoLambda = Function.Builder.create(this, "RiegoCultivoLambda")
        .runtime(Runtime.JAVA_11)
        .handler("com.myorg.lambda.RiegoCultivoLambda::handleRequest") // <-- Asegúrate que coincida con tu clase
        .code(Code.fromAsset("ruta/a/tu/target/lambda-jar.jar")) // Puede ser target/etl-cultivo-sensores.jar si lo empaquetas todo ahí
        .memorySize(512)
        .timeout(Duration.seconds(10))
        .environment(Map.of(
            "SECRET_NAME", "NOMBRE_DEL_SECRET"
        ))
        .build();
        ...
        grantPrivilegesLambda.getRole().addManagedPolicy(
            ManagedPolicy.fromAwsManagedPol@@icyName("AmazonRDSFullAccess"));
        grantPrivilegesLambda.getRole().addManagedPolicy(
            ManagedPolicy.fromAwsManagedPolicyName("SecretsManagerReadWrite"));
