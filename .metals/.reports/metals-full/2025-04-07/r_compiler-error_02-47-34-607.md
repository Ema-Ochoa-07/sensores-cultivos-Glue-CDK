...
public class EtlCultivoSensoresStack extends Stack {
    ...
    public EtlCultivoSensoresStack(final Construct scope, final String id, final StackProps props) {
        ...
        // 8. s3lamda scripts
        // ==================================
        // 1. Usa un nombre único para tu bucket
        String glueScriptsBucketName = "mis-glue-scripts-" + accountId + "-" + region;

        // 2. Configuración robusta del bucket
        Bucket scriptsBucket = Bucket.Builder.create(this, "GlueScriptsBucket")
            .bucketName(glueScriptsBucketName)
            .versioned(false)
            .autoDeleteObjects(true) // Elimina objetos automáticamente al borrar el stack
            .removalPolicy(RemovalPolicy.DESTROY) // Borra el bucket al eliminar el stack
            .blockPublicAccess(BlockPublicAccess.BLOCK_ALL) // Máxima seguridad
            .encryption(BucketEncryption.S3_MANAGED) // Encriptación por defecto
            .build();

        // 3. Opcional: Configuración adicional para Glue
        CfnJob glueJob = CfnJob.Builder.create(this, "MyGlueJob")
            // ... otras configuraciones
            .defaultArguments(Map.of(
                "--extra-jars", "s3://" + glueScriptsBucketName + "/jars/",
                "--scripts", "s3://" + glueScriptsBucketName + "/scripts/"
            ))
            .build();
        ...
        // =====================================
        new JobGlueConectRDS(this, "GlueJobExtraccion", glueRole, rdsSensores, buc@@ketSalida, accountId);
