...
public class JobGlueConectRDS extends Construct {
    ...
    public JobGlueConectRDS(final Construct scope, final String id, IRole glueJobRole) {
        ...
        // 1. Referencia al Secret generado por RDS (ajusta el nombre si es necesario)
        ISecret rdsSecret = Secret.fromSecretNameV2(this, "CultivoRDSSecret", "EtlCultivoSensoresStackRdsS-LxRUE0C2O2gm");

        // 2. Argumentos requeridos por el script de Glue
        Map<String, Object> defaultArgs = new HashMap<>();
        defaultArgs.put("--class", "GlueApp"); // si tu script Scala usa clase principal
        ...
        CfnJob glueJob = CfnJob.Builder.create(this, "GlueJobExtraccionRDS")
            ...
            .command(CfnJob.JobCommandProperty.builder()
                .name("glueetl")
                .scriptLocation("s3://datos-cultivo-procesados/scripts/extraccion_rds_cultivo.scala")@@ // usa aquí tu ruta real en S3
                .pythonVersion("3") // requerido aunque uses Scala
                .build())
